...
namespace Mautic\MauticSmsAmazonBundle\Model;
...
use Mautic\MauticSmsAmazonBundle\Api\AbstractSmsApi;
use Mautic\MauticSmsAmazonBundle\Entity\Sms;
use Mautic\MauticSmsAmazonBundle\Entity\Stat;
use Mautic\MauticSmsAmazonBundle\ModelBundle\Event\SmsEvent;
use Mautic\MauticSmsAmazonBundle\ModelBundle\Event\SmsAmazonSendEvent;
 ...
 * {@inheritdoc}
 */
class SmsAmazonModel extends FormModel implements AjaxLookupModelInterface
{
    /**
     * @var TrackableModel
     */
    ...
    {
        $q->join('t', MAUTIC_TABLE_PREFIX.'sms_amazon_messages', 's', 's.id = t.sms_id')
    ...
    public function getHitsLineChartData($unit, \DateTime $dateFrom, \DateTime $dateTo, $dateFormat = null, $filter = [], $canViewOthers = true)
    {
        ...
        if (!$flag || $flag === 'total_and_unique') {
            $q = $query->prepareTimeDataQuery('sms_amazon_message_stats', 'date_sent', $filter);
            ...
            $chart->setDataset($this->translator->trans('mautic.smsamazon.show.total.sent'), $data);
        }
