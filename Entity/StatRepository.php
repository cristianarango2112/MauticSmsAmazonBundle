...
namespace Mautic\MauticSmsAmazonBundle\Entity;
...
class StatRepository extends CommonRepository
{
    ...
    public function getSentStats($smsId, $listId = null)
    {
        ...
        $q->select('s.lead_id')
            ->from(MAUTIC_TABLE_PREFIX.'sms_amazon_messages_stats', 's')
            ->where('s.sms_id = :sms')
    ...
    public function getSentCount($smsIds = null, $listId = null)
    {
        ...
        $q->select('count(s.id) as sent_count')
            ->from(MAUTIC_TABLE_PREFIX.'sms_amazon_message_stats', 's');
    ...
    public function getLeadStats($leadId, array $options = [])
    {
        ...
        $query->select('IDENTITY(s.sms) AS sms_id, s.id, s.dateSent, e.title, IDENTITY(s.list) AS list_id, l.name as list_name, s.trackingHash as idHash')
            ->leftJoin('MauticSmsAmazonBundle:Sms', 'e', 'WITH', 'e.id = s.sms')
    ...
    {
        $this->_em->getConnection()->delete(MAUTIC_TABLE_PREFIX.'sms_amazon_message_stats', ['id' => (int) $id]);
    }
